const labKes = require('../models/LabKes')
...
class LabKesController {
    index(req, res) {
        ...
        const labKesObject = new labKes()
        labKesObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
    ...
    show(req, res) {
        const labKesObject = new labKes()
        labKesObject.show(req.params.id, req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = LabKesController